...
- [x] Séparer forme et placement dans `MassifShape` (`draw()` / `place_cliff_foot_at_origin()` / `place_at()`), à résultat identique. C'est ce qui manquait pour qu'un second massif soit possible : le placement du principal est une résolution sur la bouche de grotte, pas un réglage. **Douze graines identiques.**
- [x] Composer les reliefs en `max()` et non en somme, sur les trois composantes (hauteur, masque de falaise, influence). À un massif c'est l'identité, donc introduit **sous empreinte** : douze graines identiques.
- [x] **Contrefort** — premier contributeur de relief. `MassifShape.draw_spur(cfg, rng, wobble, profile, host)` tire et place un éperon perpendiculaire à la crête, centré sur le pied de falaise de l'hôte. Tiré en fin de chaîne de hasard : à `spur_count = 0`, empreinte identique. Validé à l'œil sur les graines 1234, 9999 et 1 — la géographie au sol change nettement, et à quatre éperons la carte bascule vers un paysage de percées rocheuses.
- [ ] `spur_count` tiré à la graine plutôt que réglé, une fois la fourchette de rendu connue : c'est le levier de variation le moins cher qu'on ait vu, une carte à un éperon et une carte à quatre ne se ressemblent pas.
...
- **Un éperon peut couper le tracé de la rivière.** Observé à quatre éperons sur la graine 1234 : le creusement s'arrête avant le bord de la carte et laisse un fossé borgne. Sans conséquence tant qu'il est sous le niveau de l'eau — c'était le cas — mais rien ne le garantit à une autre graine. C'est le premier couplage réel entre une feature et le reste de la chaîne, et c'est un **invariant** à écrire (« la rivière atteint l'aval ») plutôt qu'un cas à rattraper.
- **Rien n'empêche deux éperons de se recouvrir.** Le registre de réservation d'emprise de la passe D n'existe pas ; à `spur_count` élevé, deux éperons peuvent naître au même endroit. Visuellement anodin au `max`, mais c'est la première demande concrète du registre.
- **Les fourchettes du contrefort vivent dans `TerrainGenConfig`** et non dans un `.tres` de feature. Assumé tant qu'il n'y a qu'un type de contributeur de relief ; à déplacer quand le catalogue existera. Dette nommée à sa création, rangée dans la passe qui la corrige.
