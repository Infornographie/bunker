...
└── world/
	...
	│   ├── massif_shape.gd             — (@tool) RefCounted : un massif. draw() tire une forme, place_cliff_foot_at_origin() / place_at() la placent, draw_spur(host) fait les deux pour un éperon. project(point) = projections brutes (vallée, écoulement) ; sample_at(point) = profil, seul endroit où le placement joue. La vallée n'est PAS ici
...
- **Les massifs se composent au `max`** (`HeightmapGenerator._massif_profile_at`), sur la hauteur, le masque de falaise et l'influence. `_massifs[0]` est le principal : lui seul porte le repère dont la vallée, l'écoulement, la rivière et la bouche de grotte dérivent. Un éperon ne vote pas sur la géographie de la carte.
